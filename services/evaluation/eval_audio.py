"""Real audio duration — pydub preferred, WAV native parse, byte fallback."""
...
def _duration_fallback_bytes(audio_bytes: bytes) -> float:
    """Conservative fallback when codecs unavailable (may underestimate MP3/WebM)."""
    if not audio_bytes:
        return 0.0
    return round(len(audio_bytes) / 32000.0, 3)


def compute_audio_duration_seconds(audio_bytes: bytes, mime_guess: str = "") -> Tuple[float, str]:
    """
    Returns (duration_seconds, method_label).
    Priority: stdlib WAV → pydub → byte heuristic.
    """
    if not audio_bytes:
        return 0.0, "empty"
    ...
    return _duration_fallback_bytes(audio_bytes), "fallback_bytes"
